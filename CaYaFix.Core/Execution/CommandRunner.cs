...
public sealed class CommandRunner : ICommandRunner
{
    ...
    private static readonly HashSet<string> AllowedExecutables = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "fsutil.exe", "gpupdate.exe", "ipconfig.exe", "lodctr.exe", "netsh.exe", "nslookup.exe", "ping.exe",
        "pnputil.exe", "powercfg.exe", "powershell.exe", "reagentc.exe", "reg.exe", "route.exe", "sc.exe",
        "sfc.exe", "w32tm.exe", "winmgmt.exe", "wsreset.exe"
    };
