...
if ($checkIds.Count -ne 56 -or $checkMatches.Count -ne $checkIds.Count) { Add-Failure "Expected 56 unique diagnostics; found $($checkIds.Count)." }
if ($fixIds.Count -ne 47 -or $fixMatches.Count -ne $fixIds.Count) { Add-Failure "Expected 47 unique repair actions; found $($fixIds.Count)." }
...
if ($moduleMatches.Count -ne 14) { Add-Failure "Expected 14 module registrations; found $($moduleMatches.Count)." }
$catalogTests = Get-Content -LiteralPath (Join-Path $root 'CaYaFix.Tests\ModuleCatalogTests.cs') -Raw
foreach ($expectation in 'Assert.Equal(14, modules.Count)', 'Assert.Equal(56, modules.Sum(module => module.Checks.Count))', 'Assert.Equal(47, modules.Sum(module => module.Fixes.Count))', 'Assert.Equal(8, modules.Sum(module => module.LiveTests.Count))') {
...
foreach ($requiredReadmeText in '56 diagnostic checks', '47 repair actions', '8 interactive live tests', 'https://github.com/CaYatur', 'https://cayadev.com', 'docs/screenshots/dashboard.png', 'docs/screenshots/findings.png', 'docs/screenshots/live-tests.png') {
...
Write-Host "Repository validation passed: $($english.Count) localized keys, $($svgFiles.Count) SVG icons, 14 modules, 56 diagnostics, 47 repairs, and 8 live tests." -ForegroundColor Green
